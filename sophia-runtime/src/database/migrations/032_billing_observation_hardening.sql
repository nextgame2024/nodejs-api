...
BEGIN
  ...
  END IF;
  IF TG_TABLE_NAME = 'billing_subscription_references'
    AND NEW.external_subscription_ref <> OLD.external_subscription_ref THEN
    RAISE EXCEPTION 'Billing subscription reference identity is immutable';
  END IF;
  IF TG_TABLE_NAME = 'billing_invoice_references'
    AND NEW.external_invoice_ref <> OLD.external_invoice_ref THEN
