...
describe("BusinessManagerClient", () => {
  ...
  it("returns immediately while a BUY property report is prepared", async () => {
    const fetchMock = jest.fn<typeof fetch>().mockResolvedValueOnce(jsonResponse({
      booking: {
        ...
        listingType: "sale",
        reportDelivery: {
          reportJobId: "40000000-0000-4000-8000-000000000003",
          reportStatus: "queued",
  ...
  it("rejects connector onboarding when Business Manager returns another account identity", async () => {
    global.fetch = jest.fn<typeof fetch>().mockResolvedValueOnce(jsonResponse({
      externalAccountId: "30000000-0000-4000-8000-000000000099",
    }));
    await expect(new BusinessManagerClient().verifyConnectorIdentity(
