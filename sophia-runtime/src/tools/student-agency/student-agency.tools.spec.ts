...
it("routes student enquiries through the separate endpoint", async () => {
  const search = jest.fn<StudentBusinessManagerClient["searchStudentAgencyKnowledge"]>().mockResolvedValue({ domain: "student_migration", results: [] });
  const registry = new ToolRegistry();
  for (const tool of createStudentAgencyTools({searchStudentAgencyKnowledge: search} as unknown as StudentBusinessManagerClient)) registry.register(tool);
...
it("returns unavailable without inventing rules or using rental fallbacks", async () => {
  const search = jest.fn<StudentBusinessManagerClient["searchStudentAgencyKnowledge"]>().mockRejectedValue(new Error("offline"));
  const tool = createStudentAgencyTools({searchStudentAgencyKnowledge: search} as unknown as StudentBusinessManagerClient)[0]!;
...
it("exposes official verification without allowing caller-supplied source URLs", async () => {
  const verify = jest.fn<StudentBusinessManagerClient["verifyStudentRules"]>().mockResolvedValue({status:"official_evidence",sources:[]});
  const registry = new ToolRegistry();
  for (const tool of createStudentAgencyTools({verifyStudentRules:verify} as unknown as StudentBusinessManagerClient)) registry.register(tool);
...
it("does not claim live evidence when the verification endpoint fails", async () => {
  const verify=jest.fn<StudentBusinessManagerClient["verifyStudentRules"]>().mockRejectedValue(new Error("timeout"));
  const tool=createStudentAgencyTools({verifyStudentRules:verify} as unknown as StudentBusinessManagerClient)[1]!;
...
it("offers comparisons and rejects unknown fields",async()=>{
  const compare=jest.fn<StudentBusinessManagerClient["compareStudentRules"]>().mockResolvedValue({comparisonStatus:"supported_comparison"});
  const registry=new ToolRegistry();
  for(const tool of createStudentAgencyTools({compareStudentRules:compare} as unknown as StudentBusinessManagerClient))registry.register(tool);
