  ...
  return [
    {
      ...
      inputSchema: z.object({ listingType: z.enum(["sale", "rent"]).optional(), propertyType: z.string().trim().min(2).max(80).optional(), location: z.string().trim().min(2).max(100).optional(), city: z.string().trim().min(2).max(100).optional(), suburb: z.string().trim().min(2).max(100).optional(), minBedrooms: z.number().int().min(0).max(20).optional(), maxPrice: z.number().int().positive().max(100_000_000).optional() }),
