...
export async function getParcelOverlayMapImageBufferV2({
  ...
  const combinedBounds = boundsFromRings([parcelRingRaw, ...allOverlayParts]);
  ...
  for (const eps of epsList) {
    ...
    for (const layer of preparedLayers) {
      ...
      for (const rawLine of layer.selectedLines) {
        ...
        overlayPaths.push(
          `${layerLinePrefix}${lineToEncodedPath(overlayLine)}`,
        );
      }
    }
    ...
    if (url.length > MAX_URL_LEN) {
      ...
      if (debugLabel && selectedOverlayPaths.length < overlayPaths.length) {
        console.info(
          "[static-maps][overlay-debug] trimmed overlay paths for URL budget",
          {
            debugLabel,
  ...
  for (const layer of preparedLayers) {
    ...
    if (line)
      fallbackPaths.push(`${layerLinePrefix}${lineToEncodedPath(line)}`);
  }
