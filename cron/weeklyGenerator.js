...
async function logWhereIAm() {
  try {
    ...
    console.warn(`[${nowIso()}] [DB] Could not introspect connection:`, e?.message || e);
  }
  console.log(
    `[${nowIso()}] [ENV] S3 bucket=${process.env.S3_BUCKET} region=${process.env.S3_REGION} keyId=...${
      (process.env.S3_ACCESS_KEY_ID || "").slice(-4)
    }`
  );
  console.log(
...
async function withSingletonLock(lockId, fn) {
  const { rows } = await pool.query("SELECT pg_try_advisory_lock($1) AS ok", [lockId]);
  if (!rows?.[0]?.ok) {
    console.log(`[${nowIso()}] Another weeklyGenerator run is active. Skipping.`);
    return;
  }
...
async function insertArticleWithPrompt(client, { slug, title, description, body, prompt, status }) {
  const { rows } = await client.query(
...
async function markVideoPromptUsed(client, id) {
  await client.query(`UPDATE video_prompts SET used = TRUE, updatedAt = NOW() WHERE id = $1`, [id]);
}

async function generateFromVideoPrompt(status = "published") {
  if (!SYSTEM_AUTHOR_ID) throw new Error("SYSTEM_AUTHOR_ID env var is required");
  ...
  try {
    ...
    if (!vp) {
      console.log(`[${nowIso()}] No unused video_prompts available. Nothing to do.`);
      return null;
    }
    ...
    console.log(`[${nowIso()}] [STEP] Inserting article for prompt id=${vp.id}...`);
    ...
    console.log(`[${nowIso()}] [OK] Inserted article id=${articleId} slug=${slug}`);

    // Mark used immediately once article row exists
    await markVideoPromptUsed(client, vp.id);
    console.log(`[${nowIso()}] [OK] Marked video_prompts.id=${vp.id} as used=true`);

    // Tags — fixed series tag for analytics
    const tagList = ["video-effect", "veo3", "shorts", "content-ai"];
    ...
    console.log(`[${nowIso()}] [STEP] Generating hero image from Veo prompt...`);
    const { bytes: imgBytes, mime: imgMime } = await genImageBytes(articlePrompt);
    ...
    if (process.env.DISABLE_VIDEO === "true") {
      ...
      console.log(`[${nowIso()}] [STEP] Generating teaser video from Veo prompt...`);
      try {
        const { bytes: vidBytes, mime: vidMime, durationSec } =
        ...
        console.warn(`[${nowIso()}] [WARN] Video generation skipped:`, err?.message || err);
      }
    }

    // 3) AUDIO: Ask Gemini for a short narration script from the prompt, then synthesize with Polly
    try {
      console.log(`[${nowIso()}] [STEP] Generating narration from Veo prompt...`);
      ...
      console.warn(`[${nowIso()}] [WARN] Audio generation skipped:`, err?.message || err);
    }
    ...
    console.log(`[${nowIso()}] [DB] Verified by slug:`, rows?.[0] || "(not found)");
...
async function runBatch(count = 1, status = "published") {
  console.log(`[${nowIso()}] Starting batch from video_prompts: count=${count} status=${status}`);
  for (let i = 0; i < count; i++) {
    try {
      ...
      console.error(`[${nowIso()}] Failed generating from video_prompts:`, e?.message || e);
    }
  }
}
...
const COUNT = Number((argv.find((a) => a.startsWith("--count=")) || "").split("=")[1]) || Number(process.env.GEN_COUNT || 1);
const STATUS = (argv.find((a) => a.startsWith("--status=")) || "").split("=")[1] || process.env.GEN_STATUS || "published";
...
if (ONCE) {
  await withSingletonLock(LOCK_ID, async () => {
    ...
    try {
      ...
      console.warn(`[${nowIso()}] [DB] Could not list tail of articles:`, e?.message || e);
    }
  });
  await pool.end();
  ...
  console.log(`[${nowIso()}] Weekly video_prompts cron scheduled (Sun 03:00 UTC).`);
}
